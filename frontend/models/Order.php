    ...
    public static function saveOrder($data, $type = 1)
    {
        ...
        if ($type == self::TYPE_EY) {
			...
			if ($user->state == -1) {
                ...
                return ['code' => -1, 'info' => '您的账号已冻结请联系管理员！'];
            }
		
            if ($user->blocked_account < 0 || $user->account < 0 ||$user->state == -1) {
...
/**
    ...
    public static function saveDianOrder($data,$sim_type=null)
    {
           ...
           /* if ($data['ost']/$product->unit*$product->unit_price>$data['totalMargin']) {
            ...
            if($sim_type==null){
                if (($user->blocked_account + $data['totalMargin']*$data['volHand'] + $data['totalHandlingMoney']) > $user->account) {
					return ['code' => -1, 'info' => '您的余额已不够支付，请充值！'];
                }
            }else{
                if (($user->sim_blocked_account + $data['totalMargin']*$data['volHand'] + $data['totalHandlingMoney']) > $user->sim_account) {
			...
			if($product->code == 'MAF'){
				...
				/*$singleMargin=$product->maxlost/$product->unit*$product->unit_price;*/
				$singleMargin=1000;
			}
			//file_put_contents("log.txt", json_encode($data).PHP_EOL, FILE_APPEND);
			$beishu = $data['totalMargin']*$product->unit_price/($singleMargin*$product->unit);
            ...
            if (!$order->save()) {
                ...
                {
                //$user->blocked_account += $order->deposit;
                $user->account -= ($order->fee + $order->deposit);
                $user->save(false);
                //echo 'hehe';
                //exit;
                //经纪人返点
