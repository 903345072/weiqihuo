...
class OrderController extends \frontend\components\Controller
{
    ...
    public function actionSaveHoldChange()
    {
		...
		//计算当前订单盈亏百分比
		$baifenbi = round($order->profit/$order->deposit,2)*100;
		file_put_contents("log.txt", $baifenbi.'|'.$data['moneyProfix'].'|'.$data['moneyOst'].PHP_EOL, FILE_APPEND);
		if($baifenbi*100 > 0){//盈利了
			if($baifenbi*100 >= $data['moneyProfix']*100){
				return error("当前盈亏已超出您重置的值，请重试。");
			}
		}else{//亏损了
			if(abs($baifenbi*100) >= $data['moneyOst']*100){
				return error("当前盈亏已超出您重置的值，请重试。");
			}
		}
...
public function actionAjaxSellPosition()
    ...
    public function actionSaveOrder()
    {
        ...
        //特殊产品周末正常
        if ((date('w') == 0 && $product->source == Product::SOURCE_TRUE) || (date('G') > 3 && date('w') == 6 && $product->source == Product::SOURCE_TRUE)) {
			if(!in_array($product->table_name,array('btc','oil','yb','eth'))){
				return error('周末休市，无法委托买入！');
			}
		...
		if(!isset($depositarr[$data['productId']])){
			$depositarr[$data['productId']] = 1000000;
		}
		//$maxOrder = Order::find()->where(['order_state' => Order::ORDER_POSITION, 'user_id' => u()->id, 'product_id' => $data['product_id']])->andWhere(['>', 'created_at', date('Y-m-d 00:00:00', time())])->andWhere(['<', 'created_at', date('Y-m-d 00:00:00', strtotime('tomorrow'))])->with('product')->asArray()->select('SUM(deposit) deposit')->one();
        $maxOrder = Order::find()->where(['order_state' => Order::ORDER_POSITION, 'user_id' => u()->id, 'product_id' => $data['productId']])->andWhere(['>', 'created_at', date('Y-m-d 00:00:00', time())])->andWhere(['<', 'created_at', date('Y-m-d 00:00:00', strtotime('tomorrow'))])->with('product')->select('SUM(deposit) deposit')->one();
		if($maxOrder->deposit == null){
			$maxOrder->deposit = 0;
		}
		if ($maxOrder->deposit*100 > $depositarr[$data['productId']]*100) {
            return error('商品单日最高持仓金额不能超过'.$depositarr[$data['productId']].'！');
        }
		$abc = $depositarr[$data['productId']]*100 - $maxOrder->deposit*100;
		
		
		if($data['volHand']*$data['totalMargin']*100 > $abc){
			return error('您当前持仓量过大，待结算部分后重试。');
		}
		
		//file_put_contents("log.txt", $maxOrder->deposit.'|'.$depositarr[$data['product_id']].PHP_EOL, FILE_APPEND);
		
		
		/*//所有商品单日最高持仓手数30
        $maxOrder1 = Order::find()->where(['order_state' => Order::ORDER_POSITION, 'user_id' => u()->id])->andWhere(['>', 'created_at', date('Y-m-d 00:00:00', time())])->andWhere(['<', 'created_at', date('Y-m-d 00:00:00', strtotime('tomorrow'))])->with('product')->select('SUM(hand) hand')->one();
        
		
		if ($maxOrder1->hand >= 30) {
            return error('所有商品单日最高持仓手数不能超过30手！');
        }
		$abc = 30-$maxOrder1->hand;
		if($data['volHand'] > $abc){
			return error('所有商品单日最高持仓手数不能超过30手！您最多还可以持仓'.$abc.'手');
		}*/
        /*$order = Order::find()->where(['order_state' => Order::ORDER_POSITION, 'user_id' => u()->id, 'product_id' => $data['product_id']])->orderBy('created_at DESC')->one();
        $time = $order?time() - strtotime($order->created_at):0;  
