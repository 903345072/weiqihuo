   		...
   		foreach($products as $k=>$v){
			if(is_int($k)){
				...
				if ($result) {
                    ...
                    }elseif (in_array($v['code'],['btc_btcbitstamp','btc_btcusdtusd'])){
                        $d1 = explode('=',$resultarr[0]);
                        preg_match("/([0-9]+:[0-9]+:[0-9]+)/",$d1[1],$res);
                        preg_match("/([0-9]+-[0-9]+-[0-9]+)/",$resultarr[11],$res2);
                        $time_ = $res2[0].' '.$res[0];
                        $data = [
                            'price'=>$resultarr[8],
                            'open'=>$resultarr[7],
                            ...
                            'low'=>$resultarr[5],
                            'close'=>$resultarr[7],
                            'diff'=>$resultarr[8]-$resultarr[7],
                            'diff_rate'=> round(($resultarr[8]-$resultarr[7])*100/$resultarr[7],4),
                            'time'=>$time_
                        ];
                    }elseif ($v['code'] == 'lls'){
                        $data = [
