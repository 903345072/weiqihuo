...
<body style="">
        ...
        <ul class="index-head flex col-w <?=session('sim_type')=='sim'?'moni':''?>">
                    ...
                    <li class="mid position-tab <?=session('sim_type')?>"><div><a class="active">持仓</a><a onclick="location.replace('trans-detail1?type=<?=session('sim_type')=='sim'?'sim':''?>')">结算</a></div></li>
                    <li class="ri"></li>
        </ul>
        ...
        <ul id="container" class="position-list">
            <li style="display: none">
                <div class="list-top col-1"><span></span></div>
                <div class="list-bottom flex col-1"><span class="left"><em class=""></em><em class=""></em><em class=""></em><em class=""></em><em></em><em></em><em></em><em></em></span><span class="right"><em class="col-2" style="display:inline-block;width: 100%;font-size:.1rem;margin-bottom: .02rem"></em></span></div>
            </li>
                    ...
                    success: function(data){
                        if(data.state){
                            ...
                            $.each(list, function(){
								...
								node.find('em').eq(0).text(('昵称:') + this.user.nickname);
                                node.find('em').eq(1).text(('手机:') + this.user.mobile);
                                node.find('em').eq(2).addClass(this.rise_fall == '1' ? 'col-up' : 'col-down').text((this.rise_fall == '1' ? '买涨' : '买跌') + '：' + this.hand + '手');
                                node.find('em').eq(4).text('止盈：' + this.stop_profit_point + '点');
                                node.find('em').eq(5).text('买入：' + this.price);
                                node.find('em').eq(6).text('止损：' + this.stop_loss_point + '点');
                                container.append(node);
                                node.find('em').eq(8).text(this.created_at);
                            })
                        }
                    }
                })
                            ...
                            if(data.state){
                                ...
                                for(var proNo in obj){
                                    $('[data-' + proNo + ']').each(function(){
                                        $(this).find('em').eq(7).text('当前：' + obj[proNo]);
                                        var unit = 0;
                                        ...
                                        $(this).find('em').eq(3).removeClass().addClass(unit < 0 ? 'col-down' : 'col-up').text(unit + ' = ' + money.toFixed(2));
                                    })
                                }
                            }
                        }
