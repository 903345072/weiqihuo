...
<body style="">
        ...
        <ul id="container" class="position-list">
            <li style="display: none">
                <div class="list-top col-1" style="    line-height: .2rem;"><span></span><br/><span></span><button class="fr btn-white" style="    margin-top: -0.13rem;"><a>设置止盈止损</a></button></div>
                <div class="list-bottom flex col-1"><span class="left"><em class="" style="width:33%"></em><em class="" style="width:67%"></em><em style="width:39%"></em><em style="width:61%"></em><em style="width:39%"></em><em style="width:61%"></em></span><span class="right"><em class="col-2" style="display:inline-block;width: 100%;font-size:.1rem;margin-bottom: .02rem"></em><button id="sold" class="btn-1 fb">平仓</button></span></div>
            </li>
                    ...
                    success: function(data){
                        if(data.state){
                            ...
                            $.each(list, function(){
								...
								node.data('beishu', this.beishu);
								node.data('deposit', this.deposit);
                                ...
                                //node.find('span').eq(0).text(this.product.name + '(' + this.product.code + ')');
								node.find('span').eq(1).addClass(this.rise_fall == '1' ? 'col-up' : 'col-down').text('保证金：'+this.deposit);
                                node.find('em').eq(0).addClass(this.rise_fall == '1' ? 'col-up' : 'col-down').text((this.rise_fall == '1' ? '买涨' : '买跌') + '：' + this.hand + '手');
                                node.find('em').eq(2).text('止盈：' + this.stop_profit_point + '%');
                                node.find('em').eq(3).text('买入：' + parseFloat(this.price));
                                node.find('em').eq(4).text('止损：' + this.stop_loss_point + '%');
                            ...
                            if(data.state){
                                ...
                                for(var proNo in obj){
                                    $('[data-' + proNo + ']').each(function(){
                                        ...
                                        var money = ($(this).data('pointMoney') * unit / $(this).data('unit'))* $(this).data('beishu');
										var baifenbi = ((money*100)/($(this).data('deposit'))).toFixed(2);
										
										
										
                                        $(this).find('em').eq(1).removeClass().addClass(unit < 0 ? 'col-down' : 'col-up').text(unit + ' = ' + money.toFixed(2) + '('+baifenbi+'%)' );
                                    })
                                }
                            }
                        }
