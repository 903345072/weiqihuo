  ...
  <div class="warning  col-1"> 
   <div class="col-1 col-6">
    <ul class="rule-list">
	 ...
	 <li>
        <span class="rule-list2">规格（元/手）</span>
        <span class="rule-list2"><?=$desc[9]?>丨<?=$desc[10]?>丨<?=$desc[11]?>丨<?=$desc[12]?></span>
     </li>
	 <li>
        <span>交易综合费</span>
        <span><?=$desc[13]?>丨<?=$desc[14]?>丨<?=$desc[15]?>丨<?=$desc[16]?></span>
     </li>
	 <li>
        <span class="rule-list2">最小波动单位</span>
        <span class="rule-list2"><?=$desc[2]?></span>
     </li>
     <li>
        <span>最小波动盈亏</span>
        <span><?=$desc[17]?>丨<?=$desc[18]?>丨<?=$desc[19]?>丨<?=$desc[20]?></span>
     </li>      
     <li>
        <span class="rule-list2" style="height: .7rem;padding-top: .25rem;">交易时间</span>
        <span class="rule-list2" style="height: .7rem;padding-top: .08rem;">
        <?=$desc[4]?>
        </span>
     </li>
     <li>
	  ...
	  <li>
        <span class="rule-list2">止盈止损比</span>
    ...
    <div class="cb"></div>
    <p><?=$product->name?>产品，日均波动范围：<?=$desc[1]?>点，每波动<?=$desc[6]?>点盈亏<?=$desc[7]?>元（举例：建仓1手<?=$desc[9]?>元规格，则单日盈亏范围至少在<?=$desc[8]?>元），可买涨买跌。</p>
    <br />
    ...
    <p class="rule-content">购买的<?=$product->name?>交易数量不等价格，交易综合费收取的金额不一样。<br /></p>
    ...
    <p class="rule-content"><br /></p>
    <p class="rule-title">交易下单<br /></p>
    <p class="rule-content">您所有的<?=$product->name?>交易，全部通过期货公司的API通道，下单到交易所进行对手价撮合成交。</p>
   </div> 
  </div> 
 </body>
</html>
