...
class AdminController extends \admin\components\Controller
{
    ...
    public function actionLeaderList()
    {
        ...
        $html = $query->getTable([
            ...
            'username',
            'realname' => ['type' => 'text'],
            'leader.mobile' => ['type' => 'text'],
		    ['header' => '下级总余额','value' => function ($row) {
                return User::find()->manager()->joinWith(['admin'])->andWhere(['user.is_moni'=>0,'admin.pid'=>$row->id])->select('SUM(account) account')->one()->account ?: 0;
            }],
		    ['header' => '下级总盈亏','value' => function ($row) {
				$countQuery = (new Order)->listQuery()->joinWith(['product.dataAll', 'user.parent', 'user.admin'])->manager();
                return $countQuery->andWhere(['order.order_state' => Order::ORDER_THROW,'admin.pid'=>$row->id,'order.is_moni'=>0])->select('SUM(order.profit) profit')->one()->profit ?: 0;
            }],
		    ['header' => '下级总手续费','value' => function ($row) {
                $countQuery = (new Order)->listQuery()->andWhere(['order.order_state' => Order::ORDER_THROW,'admin.pid'=>$row->id])->joinWith(['product.dataAll', 'user.parent', 'user.admin'])->manager();
                return $countQuery->andWhere(['order.order_state' => Order::ORDER_THROW,'admin.pid'=>$row->id,'order.is_moni'=>0])->select('SUM(order.fee) profit')->one()->profit ?: 0;
            }],
		    ['header' => '下级总充值','value' => function ($row) {
                $countQuery = (new UserCharge)->listQuery()->joinWith(['user.admin'])->manager();
                return $countQuery->andWhere(['charge_state'=> UserCharge::CHARGE_STATE_PASS,'admin.pid'=>$row->id])->select('SUM(amount) AS amount')->one()->amount?:0;
            }],
			...
			['header' => '下级总提现','value' => function ($row) {
                ...
                return $countQuery->andWhere(['user.is_moni' => 0,'admin.pid'=>$row->id])->select('SUM(amount) amount')->one()->amount ?: 0;
            }],
			'state',
			['header' => '查看下级','value' => function ($row) {
				$html = '<a class="parentLink" href="http://'.$_SERVER['HTTP_HOST'].'/admin/retail/list?adminUser.pid%5D='.$row->id.'">下级会员单位</a>&nbsp&nbsp&nbsp&nbsp';
				$html .= '<a class="parentLink" href="http://'.$_SERVER['HTTP_HOST'].'/admin/sale/manager-list?admin.pid%5D='.$row->id.'">下级经纪人</a>&nbsp&nbsp&nbsp&nbsp';
				$html .= '<a class="parentLink" href="http://'.$_SERVER['HTTP_HOST'].'/admin/user/list?search%5Badmin.pid%5D='.$row->id.'">下级用户</a>&nbsp&nbsp&nbsp&nbsp';
                return $html;
            }],
            ['type' => ['delete']]
        ],[
    ...
    public function actionSaveLeader($id = null)
    {
        ...
        if ($user->load() && $adminLeader->load()) {
            ...
            if ($user->saveAdmin($type)) {
                ...
                $adminLeader->save();
				$this->redirect(['/admin/admin/leader-list']);
                return false;
            } else {
                return self::error($user);
            }
