    ...
    public static function sellOrderHand($id,$isConsole=false,$sim_type=null)
    {
        ...
        if (!empty($order)) {
            ...
            if ($order->rise_fall == self::RISE) {
                ...
                $diffPrice = sprintf('%.6f', $dataAll->price - $order->price);
            } else {
            //买跌
                $diffPrice = sprintf('%.6f', $order->price - $dataAll->price);
            }
            ...
            if ($order->profit > 0) {
                if ($order->stop_profit_point > 0) {
                    //盈利不能超过设置盈利
                    if ($order->profit > $order->deposit*$order->stop_profit_point/100) {
                        //$order->profit = $order->hand*$order->one_profit*$order->stop_profit_point;
						$order->profit = $order->deposit*$order->stop_profit_point/100;
                    }
                }
            } else {
                if ($order->stop_loss_point > 0) {
                    //亏损不能超过设置亏损
                    if (-$order->profit > $order->deposit*$order->stop_loss_point/100) {
                        $order->profit = -($order->hand*$order->one_profit*$order->stop_loss_point);
						$order->profit = -($order->deposit*$order->stop_loss_point/100);
                    }
                }
            }
            //卖掉收入
            ...
            if ($order->save()) {
                ...
                else
                {
                    if ($order->profit != 0) {
                        ...
                        $balance_account = new BalanceAccount();
                        $balance_account->order_id = $id;
                        $balance_account->user_id = $order->user_id;
                        $balance_account->balance = $order->profit + $order->deposit;
                        if ($order->profit > 0) {
                            $balance_account->profit_account = $order->profit;     //+93
                        } else {
                            $balance_account->loss_account = $order->profit;
                        }
                        $balance_account->save(0);
                        return true   ;
                }
