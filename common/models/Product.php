...
class Product extends \common\components\ARModel
{
     ...
     * @return boolean
     */
    public static function isTradeTime($productId, $early = 100)
    {
        $model = self::findOne($productId);
       ...
       /* if($model->table_name == 'btc' || $model->table_name == 'oil' || $model->table_name == 'yb' ){*/
        if($model->table_name == 'btc' || $model->table_name == 'oil' || $model->table_name == 'eth' ){
			$weekend=true;
		}
        $time = $model->trade_time;
        if (!$time) {
