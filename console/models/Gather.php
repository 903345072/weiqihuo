...
class Gather extends \yii\base\Object
{
    ...
    {
		//file_put_contents("log.txt", $name.'|'.json_encode($data).PHP_EOL, FILE_APPEND);
        try {
            // 是否开启作弊模式
			...
			//file_put_contents("log33333.txt", $name.'|'.$data['price'].'|'.$data['time'].PHP_EOL, FILE_APPEND);
            if (self::dbInsert('data_' . $name, ['price' => $data['price'], 'time' => $data['time']])) {
				if(in_array($name,array('oil','btc','yb'))){
					$dataInfo = DataAll::findOne($name);
    ...
    protected function listen()
    {
       ...
       // foreach ($productMap as $product => $isTrade) {
        ...
        array_walk($ids, function ($value) {
			...
			$bili = round(abs($value['profit'])*100/$value['deposit'],2);
			
			$is_sell_order = 0;
			if($value['profit'] > 0){
				if($bili*100 >= $value['stop_profit_point']*100){
					$is_sell_order = 1;
				}
			}else{
